package com.ll.exam.sb_batch.app.base;
...
import com.ll.exam.sb_batch.app.order.service.OrderService;
import com.ll.exam.sb_batch.app.product.entity.Product;
import com.ll.exam.sb_batch.app.product.entity.ProductOption;
import com.ll.exam.sb_batch.app.product.service.ProductService;
...
public class DevInitData {
    ...
    public CommandLineRunner initData(MemberService memberService, ProductService productService, CartService cartService, OrderService orderService) {
        return args -> {
            ...
            ProductOption productOption1__RED_44 = product1.getProductOptions().get(0);
            ProductOption productOption1__BLUE_44 = product1.getProductOptions().get(2);

            cartService.addItem(member1, productOption1__RED_44, 1); // productOption__RED_4 총 수량 1
            cartService.addItem(member1, productOption1__RED_44, 2); // productOption__RED_44 총 수량 3
            cartService.addItem(member1, productOption1__BLUE_44, 1); // productOption__BLUE_44 총 수량 1

            Order order1 = orderService.createFromCart(member1);

            int order1PayPrice = order1.calculatePayPrice();
            log.debug("order1PayPrice : " + order1PayPrice);
            ...
            cartService.addItem(member2, product2Option__BLACK_44, 3);
            cartService.addItem(member2, product2Option__WHITE_44, 3);

            Order order2 = orderService.createFromCart(member2);

            log.debug("order2 payPrice : " + order2.calculatePayPrice());

            memberService.addCash(member2, 600_000, "충전__무통장입금");
